...
const WidgetsLog_ts_1 = require("./WidgetsLog.ts");
/**
...
class FileManager {
    static async readFile(path) {
        return new Promise((resolve, reject) => {
            fs.readFile(path, 'utf8', (err, data) => {
                if (err || !data) {
                    WidgetsLog_ts_1.WidgetsLog.error("Couldn't read file:" + path);
                    reject(err);
                    return;
                }
                resolve(data);
    ...
    static async getSwiftFiles(sourceDir) {
        return await (0, glob_1.glob)(`${sourceDir}*.swift`, {
            stat: true,
            withFileTypes: true,
    ...
    static async writeFile(path, contents) {
        return new Promise((resolve, reject) => {
            fs.writeFile(path, contents, 'utf8', (err) => {
                if (err) {
                    WidgetsLog_ts_1.WidgetsLog.error("Couldn't write file:" + path);
                    reject(err);
                    return;
                }
                resolve();

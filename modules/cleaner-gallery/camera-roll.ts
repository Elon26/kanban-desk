...
import { GalleryAsset, GetAssetsParams } from './types';
...
export class CameraRoll {
  ...
  static async findSimilarImages(interval = 15): Promise<GalleryAsset[][]> {
    const groups =
      await CleanerGalleryModule.findSimilarImagesFromGallery(interval);
  ...
  static async findBlurryImages({
    ...
    const { items, processedIds } =
      await CleanerGalleryModule.findBlurryImagesFromGallery(
        ignoreIds,
        threshold,
        itemsPerPage
      );

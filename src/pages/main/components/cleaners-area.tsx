...
export default function CleanersArea({
  hasPremium,
  hasButton,
  hasNewButton,
}: Props) {
  ...
  useEffect(() => {
    if (similarGroups) {
      updateWidgetData('CleanerWidget', {
        duplicateNumbers: similarGroups.flat().length,
        duplicateNumbersSize: similarGroups.flat().length * 1000,
      });
    }
  }, [similarGroups, updateWidgetData]);

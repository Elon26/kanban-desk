...
export function PaywallD() {
  const setIsReadyToShowNotification = useSetStorage(
    'isReadyToShowNotification'
  );
  const { annoyingPaywallD: annoyingPaywall, subscription_for_paywall_d } =
    useConfig();
  ...
  if (subscription_for_paywall_d === 'a') {
  ...
  if (subscription_for_paywall_d === 'b') {
    subscriptions = sub?.filter(
      (sub) =>
        sub.periodUnit !== 'week' ||
        (sub.periodUnit === 'week' &&
  ...
  if (subscription_for_paywall_d === 'c') {
    subscriptions = sub?.filter(
      (sub) =>
        sub.periodUnit !== 'week' ||
        (sub.periodUnit === 'week' &&

...
export default function SelectDeadlinePage({
  taskId,
}: {
  ...
  function handleSaveTask() {
    ...
    if (taskId) {
      if (
        userTask &&
        userTask.startTime &&
        new Date(userTask.startTime) > selectedFullDate
      ) {
        setError(langs[selectedLang].pages.task_manager.deadline_time);
        return;
      }

      const editedTasks = userTasks.map((task) => {
        if (task.id === taskId) {
          task.endTime = selectedFullDate || null;
      ...
      if (newTask.startTime && new Date(newTask.startTime) > selectedFullDate) {
        setError(langs[selectedLang].pages.task_manager.deadline_time);
        return;
      }

      setNewTask((prev) => {
        prev.endTime = selectedFullDate;
        return prev;
      });
    }
    router.back();
  ...
  return (
    <Page>
      ...
      <Container>
        <PageHeader
          pageName={langs[selectedLang].page_names.deadline}
          ...
          <View className="rounded-3xl bg-white/10 gap-y-3 px-3 mb-10 mt-2.5 py-4">
            ...
            <View className="rounded-3xl bg-white/10 p-4">
              <View className="flex-row gap-x-2 mb-2.5">
                ...
                <UiText className="font-medium color-gray">
                  {langs[selectedLang].pages.task_manager.select_deadline}
                </UiText>
              </View>
              <Calendar
                style={{
            ...
            <View className="rounded-3xl bg-white/10 p-4">
              <View className="flex-row gap-x-2">
                <ClockIcon />
                <UiText className="font-medium color-gray">
                  {langs[selectedLang].pages.task_manager.set_deadline}
                </UiText>
              </View>
              <DateTimePicker
                value={selectedTime}
